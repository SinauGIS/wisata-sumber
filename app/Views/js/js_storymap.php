  ...
  $(window).on('load', function() {
    ...
    function initMap(options, chapters) {
      ...
      if (getSetting('_mapLogo')) {
        $('#logo').append('<img src="<?= base_url('public/storymaps') ?>/' + getSetting('_mapLogo') + '" />');
        $('#top').css('height', '60px');
      } else {
        ...
        for (var i = 0; i < pixelsAbove.length - 1; i++) {
          ...
          if (currentPosition >= pixelsAbove[i] &&
            ...
            if (c['GeoJSON Overlay']) {
              let url_geojson = '<?= base_url('public/storymaps') ?>/' + c['GeoJSON Overlay'];

              $.getJSON(url_geojson, function(geojson) {
    ...
    function changeAttribution() {
      ...
      var name = 'SINAUGIS';
      var url = 'https://www.sinaugis.com/';
